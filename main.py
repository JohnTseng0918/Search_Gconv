...
def get_args():
    ...
    parser.add_argument("--supernet_epoch", default=100, help="train supernet epoch", type=int)
...
def main():
    ...
    nas.load_model()
    nas.get_dataloader()
    nas.show_init_model_info()
    nas.init_supernet()
    nas.grow_supernet()
    nas.grow_supernet()
    nas.pretrained_to_all_supernet()

    nas.train_supernet(args.supernet_epoch)
    nas.search()
    nas.genome_build_model(nas.select_from_topk())
    nas.print_genome()
    flops, params = nas.count_flops_params()
    print("model conv and linear flops:", flops)
    print("model parameters:", params)
    nas.fine_tune(lr=0.001, ftepoch=10, testepoch=1)
